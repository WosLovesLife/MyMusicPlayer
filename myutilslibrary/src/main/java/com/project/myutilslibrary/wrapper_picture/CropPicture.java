...
public class CropPicture {
    ...
    public static Bitmap getScaledDrawable(String resFile, float destWidth, float destHeight, Bitmap.Config mode) {
        if (destWidth < 1) destWidth = 1;
        if (destHeight < 1) destHeight = 1;
        ...
        BitmapFactory.Options options = getBoundOption();
        /* 用位图工厂析出一个只有边框大小的数据的Option对象 */
        BitmapFactory.decodeFile(resFile, options);
        ...
        return BitmapFactory.decodeFile(resFile, options);
    }
